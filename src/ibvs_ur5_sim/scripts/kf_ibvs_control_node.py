...
class CompensatedIBVSController:
    def __init__(self):
        ...
        self.corners_sub = message_filters.Subscriber('/aruco_corners', PoseArray)
        self.depth_sub = message_filters.Subscriber('/aruco_corners_pseudo_depth', Float32MultiArray)
        
        self.ts = message_filters.ApproximateTimeSynchronizer(
            [self.corners_sub, self.depth_sub], 
            queue_size=10, 
            slop=0.5,
    ...
    def update_filter_from_past(self, z, measurement_time):
        ...
        with self.lock:
            ...
            past_pred_x = self.history_buffer[target_index]['state'].copy()
            past_pred_p = self.history_buffer[target_index]['covariance'].copy()
    ...
    def control_loop_callback(self, event):
        ...
        rospy.loginfo(f"Current Avg Pixel Error: {avg_pixel_error:.4f}, Predicted Corner (u1,v1): {s_cur_predicted[0]:.2f}, {s_cur_predicted[1]:.2f}")
        
        self.error_pub.publish(avg_pixel_error)
        
        if avg_pixel_error < self.error_threshold_:
            rospy.loginfo(f"Target reached! Error: {avg_pixel_error:.3f}")
            self.servoing_active = False
            return
        ...
        #     if Z <= 0: 
        #         rospy.logwarn(f"Invalid depth Z={Z} for point {i}. Skipping control command.")
        #         return
